tiffline_ChartUI <- function(id, in_out_type = NULL) {
  ns <- NS(id)
  
  base_note <- HTML("<strong>Note:</strong><ul>
    <li>Each variable has been numbered. The totals include information in brackets about how they have been derived from the other variables.</li>
  ...
  tiff_note <- HTML("<p><strong>Each variable has been numbered. The totals include information in brackets about how they have been derived from the other variables</p>")
  
  note_content <- if (in_out_type == "tiff") {tiff_note} 
  else NULL
  
  
  tagList(
    htmlOutput(ns("title")),
    highchartOutput(ns("tiffline_chart")),
    note_content,
...
tiffline_ChartServer <- function(id, chart_data, title, yAxisTitle, xAxisTitle, unit, footer, x_col, y_col) {
  moduleServer(id, function(input, output, session) {
    ...
    output$tiffline_chart <- renderHighchart({
      data <- chart_data()
