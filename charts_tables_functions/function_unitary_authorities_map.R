...
unitary_authorities_geojson_data <- geojson_read("utility/unitary_authorities_simplified.geojson", what = "sp")
...
mapUnitaryServer <- function(id, data, variable, unit = "", title, footer, legend_title = "Legend") {
  moduleServer(id, function(input, output, session) {
    ...
    output$map_uni <- renderHighchart({
      data <- filtered_data()
      
      hc_data <- data %>% 
        mutate(unitauth = as.character(unitauth)) %>%
        select(unitauth, value) %>%
        list_parse()
      ...
      highchart(type = "map") %>%
        hc_add_series(
          mapData = unitary_authorities_geojson_list, 
          joinBy = c("local_auth", "unitauth"), 
          data = hc_data,
          borderColor = "#FFFFFF",
          borderWidth = 0.5,
          states = list(
